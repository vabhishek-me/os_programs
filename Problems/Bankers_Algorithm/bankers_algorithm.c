...
int main(int argc, char** argv) {
	...
	// run threads
	THREAD *processes = (THREAD *)malloc(nProcesses * sizeof(*processes));
	for(int i=0; i<nProcesses; i++) processes[i].tid = i;

        pthread_attr_t attr;
        pthread_attr_init(&attr);

        for(int i=0; i<nProcesses; i++)
                pthread_create(&processes[i].thread, &attr, processCode, (void *)(&processes[i]));
